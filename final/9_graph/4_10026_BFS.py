...
def breadth_first_search(graph, row, col):
    ...
    """
    queue = deque()
    queue.append((row, col))      # 큐에 시작 좌표를 넣는다

    move = [[0, -1], [0, 1], [-1, 0], [1, 0]]   # 좌, 우, 상, 하 이동 방향

    color = graph[row][col]        # 현재 영역의 색 저장
    graph[row][col] = 'X'          # 시작 좌표를 방문 표시('X'로 덮음)

    while len(queue)>0:
        row, col = queue.popleft()                 # 큐에서 하나를 꺼냄
        for m in range(4):
            next_row, next_col = row, col
            next_row += move[m][0]
            next_col += move[m][1]

            # 이동할 좌표가 유효한지, 같은 색인지, 방문되지 않았는지 확인
            if not check_move(graph, next_row, next_col, color):
                continue
            queue.append((next_row, next_col))             # 조건이 충족되면 큐에 추가
...
for i in range(size):
    for j in range(size):
        if graph_colorblind[i][j] != 'X':
            breadth_first_search(graph_colorblind, i, j)
            count_colorblind += 1

# BFS 버전 결과 출력: 일반 시야의 영역 수, 적록색약 시야의 영역 수
